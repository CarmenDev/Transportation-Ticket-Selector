        ...
        <div className="conclusionSection">
            <div>
                {touristNumber === 1 ? <p>
                    ...
                    You are part of a group of <span>{touristNumber}</span> people.
                </p>}
                {isStudent && <p>and you are a student.</p>}
                <p>You are going to stay in Berlin for <span>{days}</span> day(s).</p>
                {bikes !==0 && <p>You will take <span>{bikes}</span> bike(s) along</p>}
            </div>
            
            <p>The total price is <span>{ticketTotalPrice}</span> Euro.</p>
            <p>You need to buy the following number of tickets for you or your group:</p>
            <div>
            {dayTicketNumber > 0 && 
                <p>
                    <span>{dayTicketNumber}</span> single day ticket(s)
                </p>
            }
            {sevenDayTicketNumber > 0 && 
                <p>
                    <span>{sevenDayTicketNumber}</span> seven days ticket(s)
                </p>
            }
            {monthlyTicketNumber > 0 && 
                <p>
                    <span>{monthlyTicketNumber}</span> monthly ticket(s)
                </p>
            }
            {studentMonthlyTicketNumber > 0 && 
                <p>
                    <span>{studentMonthlyTicketNumber}</span> monthly ticket(s) for students
                </p>
            }
            {dayBikeTicketNumber > 0 && 
                <p>
                    <span>{dayBikeTicketNumber}</span> single day ticket(s) for bikes
                </p>
            }
            {monthlyBikeTicketNumber > 0 && 
                <p>
                    <span>{monthlyBikeTicketNumber}</span> monthly ticket(s) for bikes
                </p>
            }
            {yearlyTicketNumber > 0 && 
                <p>
                    <span>{yearlyTicketNumber}</span> yearly ticket(s)
                </p>
            }
            {groupTicketNumber > 0 && 
                <p>
                    <span>{groupTicketNumber}</span> group ticket(s)
                </p>
            }
            </div>
        </div>
